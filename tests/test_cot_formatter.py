...
from cot_formatter import CotFormatter
from models import GeoLocation
from fixture import sample_geolocation_fixed, sample_cot_event_fixed, fixed_datetime
...
def test_event_type_mapping(formatter):
    ...
    assert (
        formatter._get_event_type("emergency") == CotFormatter.EVENT_TYPES["emergency"]
    )
...
def test_how_value_mapping(formatter):
    ...
    assert (
        formatter._get_how_value("calculated") == CotFormatter.HOW_VALUES["calculated"]
    )
...
def test_create_event(formatter, sample_geolocation_fixed):
    ...
    assert event.point == sample_geolocation_fixed
    assert event.time == sample_geolocation_fixed.timestamp
    assert event.start == sample_geolocation_fixed.timestamp
    assert event.stale == sample_geolocation_fixed.timestamp + datetime.timedelta(
        minutes=5
    )


def test_format_event(formatter, sample_cot_event_fixed):
    xml_bytes = formatter.format_event(sample_cot_event_fixed)
    ...
    assert root.get("type") == sample_cot_event_fixed.event_type
    assert root.get("uid") == sample_cot_event_fixed.event_id
    assert root.get("how") == sample_cot_event_fixed.how
    assert root.get("time") == formatter._format_datetime(sample_cot_event_fixed.time)
    assert root.get("start") == formatter._format_datetime(sample_cot_event_fixed.start)
    assert root.get("stale") == formatter._format_datetime(sample_cot_event_fixed.stale)

    point = root.find("point")
    assert point is not None
    assert float(point.get("lat")) == sample_cot_event_fixed.point.lat
    assert float(point.get("lon")) == sample_cot_event_fixed.point.lon
    assert float(point.get("hae")) == sample_cot_event_fixed.point.hae
    assert float(point.get("ce")) == sample_cot_event_fixed.point.ce
    assert float(point.get("le")) == sample_cot_event_fixed.point.le
...
def test_xml_declaration(formatter, sample_cot_event_fixed):
    xml_bytes = formatter.format_event(sample_cot_event_fixed)
    xml_str = xml_bytes.decode("utf-8")
...
def test_create_event_defaults(formatter, sample_geolocation_fixed):
    event = formatter.create_event(sample_geolocation_fixed)
